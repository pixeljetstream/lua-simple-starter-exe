...
#define HandleError MessageBox

#else

int HandleError( HWND hWnd, LPCTSTR lpText, LPCTSTR lpCaption, UINT uType )
{
  fprintf(stderr, "%s:\n%s\n", lpCaption, lpText);
  return 1;
}
...
static int luafunc_mbox (void *L)
{
  ...
  HandleError(NULL,msg,title,MB_OK|MB_ICONERROR);
  return 0;
}


static const char *luacode =
"local startup = _STARTUP; _STARTUP = nil; "
"local msg = _ERRMSG; _ERRMSG = nil; "
"local arg = _ARG or {}; _ARG = nil; "
"local hasext = startup:match('(.+)%..-$'); "
"startup = (hasext or startup)..'.lua'; "
"xpcall("
"function() "
"(loadfile (startup))(unpack(arg)) end,"
"function(err) msg('Uncaught lua script exception',startup..debug.traceback(err)) end)"
;
...
int main (int argc, char *argv[])
{
  ...
  char bufferstart[MAX_PATH];
  char *startfile = "init.lua";
  ...
  if (file!=NULL){
    strcpy_s(bufferstart, MAX_PATH, file);
  ...
  if (hinstLib != NULL)
  {
      ...
      lua_createtable && lua_pushstring && lua_rawseti)
    {
      ...
      if (L!=NULL) {
        ...
        lua_setfield(L,LUA_GLOBALSINDEX,"_STARTUP");
